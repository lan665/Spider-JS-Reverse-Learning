...
经过promise.then的上下栈scope分析，确认加密参数生成位置就在拦截器中，加密参数是在进入底层调度之前的请求拦截器中生成的，直接进入initiator中的Promise.then下方一个栈的 o = o.then(v.shift(), v.shift())。
此处如果单步执行会陷入 Webpack 的混淆泥潭。
