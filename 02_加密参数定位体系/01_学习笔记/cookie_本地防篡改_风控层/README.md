...
完成溯源跨越： 由于 jeyxP 这一层只是个参数传递中转站（密文在传入时已经生成完毕），我们必须如图中的红色箭头所示，向下点击第一个 (匿名) 函数栈帧。这一步操作如同穿透了重重迷雾，直接跳回了生成这段密文的真正业务逻辑入口。
...
当我们顺着调用栈继续深入，试图寻找类似前几节中清晰的 AES.encrypt 时，迎面而来的却是：
