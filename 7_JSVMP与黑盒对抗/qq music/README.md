...
**难点**：拿到二进制密文后，在 Python 中无法解密，必须调回 JS 环境的 `se()` 函数。但由于前期补环境脚本污染了 Node.js 的原生对象，导致传入二进制流时频繁报错。
...
## 3. 核心经验与心法沉淀 (Key Takeaways)

### 认知升级 1：不要和 JSVMP 拼体力
...
当你看到 `regeneratorRuntime.mark` 和 `switch (n.prev = n.next)` 时，不要慌张。这不是故意恶心你的混淆，而是 Webpack 打包时为了兼容低版本浏览器，将 `async/await` 语法强行转译成了状态机。遇到这种代码，核心关注 `n.next = xxx` 以及 `n.sent`（代表 await 返回的结果）。
